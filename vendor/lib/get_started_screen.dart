...
import 'vendor_type_selection_screen.dart';
      ...
      PageRouteBuilder(
        pageBuilder: (context, animation, secondaryAnimation) => _PhoneInputFullScreen(
          ...
          onContinue: (String phone) {
            print('onContinue called with phone: $phone'); // Debug log
            final fullPhone = '+233$phone';
            print('Full phone number: $fullPhone'); // Debug log
            // First pop the modal, then navigate to vendor type selection
...
class _PhoneInputFullScreenState extends State<_PhoneInputFullScreen> {
  ...
  void _handleContinue() {
    ...
    // More lenient validation - accept any number with 9+ digits
    if (digits.isEmpty || digits.length < 9) {
      setState(() => _phoneError = 'Enter a valid phone number (at least 9 digits)');
      Haptics.error();
      return;
    }
    
    // Ensure we have exactly 9 digits for Ghana numbers
    if (digits.length > 9) {
    ...
    Future.delayed(const Duration(milliseconds: 1200), () {
      if (mounted) {
        ...
        print('Phone validation passed, calling onContinue with: $digits');
