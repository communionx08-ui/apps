...
class _GetStartedScreenState extends State<GetStartedScreen>
  ...
  void _handleGoogleSignIn() async {
    ...
    try {
      ...
      // Simulate successful Google sign in - now need phone
      Navigator.push(
        context,
        PageRouteBuilder(
          pageBuilder: (context, animation, secondaryAnimation) => const PhoneCollectionScreen(
      ...
      if (mounted) {
        _showComingSoon('Google Sign-In failed. Please try again.');
      }
    } finally {
      if (mounted) {
  ...
  void _handleAppleSignIn() async {
    ...
    try {
      ...
      // Simulate successful Apple sign in - now need phone
      Navigator.push(
        context,
        PageRouteBuilder(
          pageBuilder: (context, animation, secondaryAnimation) => const PhoneCollectionScreen(
            name: 'Jane Smith', // From Apple
      ...
      if (mounted) {
        _showComingSoon('Apple Sign-In failed. Please try again.');
      }
    } finally {
      if (mounted) {
        setState(() => _isLoading = false);
            ...
            children: [
              ...
              SwiftButton(
                ...
                onPressed: () async {
                  ...
                  if (email.isEmpty) {
                    setModalState(() => emailError = 'Please enter your email');
                    return;
                  }
                  if (!RegExp(r'^[^@]+@[^@]+\.[^@]+').hasMatch(email)) {
                    setModalState(() => emailError = 'Please enter a valid email');
                    return;
                  }
                  ...
                  Navigator.push(
                    context,
                    PageRouteBuilder(
                      pageBuilder: (context, animation, secondaryAnimation) => EmailVerificationScreen(email: email),
                      transitionDuration: const Duration(milliseconds: 500),
                      transitionsBuilder: (context, animation, secondaryAnimation, child) {
                        return FadeTransition(
                          opacity: CurvedAnimation(
                          ...
                          builder: (context) => Container(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                _SocialButton(
                                  icon: const Icon(Icons.phone, color: AppColors.primary, size: 24),
                                ...
                                _SocialButton(
                                  icon: const Icon(Icons.email, color: AppColors.primary, size: 24),
